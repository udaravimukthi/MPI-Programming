...
int main(int argc, char* argv[])
{
	...
	/* Assertation check for wether argumnent is given or
	...
	// Calculate the size of the array required by each process
	size = length / comm_sz;
	// Open up space for the array used by the process
	sub = (int*)malloc(size * sizeof(int));
	// Distribute the array elements to each process
	...
	if (my_rank == 0) {
		// Open up the array space required for the result(as large as the input array)
		result = (int*)malloc(length * sizeof(int));
	}
	// collect the array sorted by each process to process 0
	MPI_Gather(sub, size, MPI_INT, result, size, MPI_INT, 0, MPI_COMM_WORLD);
	if (my_rank == 0) {
		tmp_for_last = (int*)malloc(length * sizeof(int));
